...
> **V2.1 核心原则：不重复造轮子，几何正确优先，输入语义分叉**
> - 彩色填充图：矢量化复用 **VTracer**，几何核心复用 **Shapely**（Region → SharedBoundary）
> - 黑白线稿：骨架化复用 **scikit-image**（Stroke → Skeleton 中心线），彻底解决粗线变双线问题
> - 自动检测输入类型，不确定时优先彩色模式
...
图片
  │
Input Classifier (自动检测: 颜色数/饱和度/黑白比例/灰度标准差)
  │
  ├─ 彩色填充图 ──────────────────────────────────────────────┐
  │                                                             │
  ▼                                                             ▼
┌────────────────────────────────────────────────┐  ┌──────────────────────────────┐
│  VTracer (开源矢量化引擎)                        │  │  BW 二值化 (OpenCV Otsu)     │
│  图片 → 颜色区域 → 初始Spline矢量曲线            │  │  + 中值去噪 + 边界清除        │
└────────────────────────────────────────────────┘  └──────────────────────────────┘
  │ SVG解析 → 矢量Region (Polygon)                   │ mask (True=黑线前景)
  ▼                                                   ▼
┌────────────────────────────────────────────────┐  ┌──────────────────────────────┐
│  Shapely (开源几何库)                            │  │  scikit-image skeletonize     │
│  Region Polygon → 邻接判定 → buffer容差交         │  │  粗笔画 → 单像素宽骨架        │
│  → Shared Boundary LineString                   │  └──────────────────────────────┘
└────────────────────────────────────────────────┘    │
  │ label_map 相邻过滤 + 重叠裁剪 + 端点焊接          ▼
  ▼                                              ┌──────────────────────────────┐
┌────────────────────────────────────────────────┐  │  Skeleton Graph (8邻域)     │
│  工程验证 (CurveValidator / Shapely真几何)       │  │  endpoint/normal/junction   │
│  自交 is_simple / 线距 distance / 小半径         │  │  → Edge 有序点串             │
│  曲线合并 (CurveMerger 4端点+自动翻转, G0/G1)    │  └──────────────────────────────┘
│  断线修复                                       │    │ Spur Pruning (迭代修剪<0.8mm)
└────────────────────────────────────────────────┘    ▼
  │                                              ┌──────────────────────────────┐
  │                                              │  坐标转换 → 简化 → Bezier    │
  │                                              │  → G0/G1合并 → 工程验证       │
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `min_spur_length_mm` | 0.8 | 毛刺修剪阈值，短于此长度的末端分支被删除 |
| `keep_fine_segments` | false | 保留细小线段（如眼睫毛、花蕊），关闭毛刺修剪 |
